...
type TableHerokuTeamFeaturesGenerator struct {
}

var _ table_schema_generator.TableSchemaGenerator = &TableHerokuTeamFeaturesGenerator{}

func (x *TableHerokuTeamFeaturesGenerator) GetTableName() string {
	return "heroku_team_features"
}

func (x *TableHerokuTeamFeaturesGenerator) GetTableDescription() string {
	return ""
}

func (x *TableHerokuTeamFeaturesGenerator) GetVersion() uint64 {
	return 0
}

func (x *TableHerokuTeamFeaturesGenerator) GetOptions() *schema.TableOptions {
...
func (x *TableHerokuTeamFeaturesGenerator) GetDataSource() *schema.DataSource {
	return &schema.DataSource{
		Pull: func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask, resultChannel chan<- any) *schema.Diagnostics {
			...
			for _, it := range items {
				...
				for nextRange.Max != 0 {
					ctxWithRange := context.WithValue(ctx, "nextRange", nextRange)
					v, err := c.TeamFeatureList(ctxWithRange, it.ID, nextRange)
					if err != nil {
						maybeError := errors.WithStack(err)
						return schema.NewDiagnosticsErrorPullTable(task.Table, maybeError)
					}
...
func (x *TableHerokuTeamFeaturesGenerator) GetExpandClientTask() func(ctx context.Context, clientMeta *schema.ClientMeta, client any, task *schema.DataSourcePullTask) []*schema.ClientTaskContext {
	return nil
}

func (x *TableHerokuTeamFeaturesGenerator) GetColumns() []*schema.Column {
...
func (x *TableHerokuTeamFeaturesGenerator) GetSubTables() []*schema.Table {
	return nil
}
